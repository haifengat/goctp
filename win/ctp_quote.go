...
type qOnRspQryMulticastInstrumentType func(*ctp.CThostFtdcMulticastInstrumentField, *ctp.CThostFtdcRspInfoField, int, bool) uintptr
...
// 请求查询组播合约响应
func (q *quote) regOnRspQryMulticastInstrument(on qOnRspQryMulticastInstrumentType) {
	_, _, _ = q.h.MustFindProc("SetOnRspQryMulticastInstrument").Call(q.spi, syscall.NewCallback(on))
}
...
// 请求查询组播合约
func (q *quote) ReqQryMulticastInstrument(pQryMulticastInstrument ctp.CThostFtdcQryMulticastInstrumentField) {
	q.nRequestID++
	_, _, _ = q.h.MustFindProc("ReqQryMulticastInstrument").Call(q.api, uintptr(unsafe.Pointer(&pQryMulticastInstrument)), uintptr(q.nRequestID))
}
